  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      try {
        ...
        if (additionalCharges > 0) {
          const chargeDetails = [];
          if (extraCharges > 0)
            chargeDetails.push(`Extra Services: ${extraCharges}`);
          if (minibarCharges > 0)
            chargeDetails.push(`Minibar: ${minibarCharges}`);
          if (damageCharges > 0) chargeDetails.push(`Damage: ${damageCharges}`);
          if (lateCheckoutFee > 0)
            chargeDetails.push(`Late Checkout: ${lateCheckoutFee}`);
          ...
          await tx.reservation.update({
            ...
            data: {
              remarks: `${
                updatedReservation.remarks || ""
              }\n\nAdditional Charges (${new Date().toISOString()}): ${chargeDetails.join(
                ", "
    ...
    if (error instanceof Error) {
      if (error.message.includes("Foreign key constraint")) {
        ...
        errorMessage =
          "Check-out conflict. This reservation may already be processed.";
      } else if (error.message.includes("Transaction failed")) {
        errorMessage = error.message;
      } else {
        errorMessage = error.message;
